...
const AsignacionesAuditado = () => {
  ...
  const id_auditado = sessionStorage.getItem('id'); // Obtener el nombre del usuario del sessionStorage

  const fetchAsignaciones = () => {
    console.log(id_auditado);
    fetch(`http://localhost:3001/auditadoauditorias/${id_auditado}`) // Pasar el nombre del usuario en la URL
      .then((response) => {
        if (!response.ok) {
                ...
                <tr key={asignacion.id}>
                  ...
                  <td>
                    <div className="acciones">
                      <a href={`/Auditadoinconformidad/${asignacion.id}`}>
...
export default AsignacionesAuditado;
